...
def get_hypotheses(num_batches, num_samples, sess, tensor):
    '''Gets hypotheses.
    num_batches: scalar.
    num_samples: scalar.
...
#         print(12121212, tensor.get_shape())
        h = sess.run(tensor)
        hypotheses.extend((h.tolist()))
#     hypotheses = postprocess(hypotheses)
    hypotheses_str = []
    for i in range(len(hypotheses)):
        hypotheses_str.append(str(hypotheses[i]))
    return hypotheses_str[:num_samples], hypotheses[:num_samples]
